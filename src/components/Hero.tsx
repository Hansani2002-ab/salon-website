...
import { CheckCircle2, Scissors, Armchair, MapPin, ClipboardList, ArrowRight } from 'lucide-react';
...
const Hero = () => {
            ...
            {categories.map((cat, index) => (
              <div key={index} className="flex flex-col items-center group cursor-pointer">
                ...
                <div className="text-center">
                  ...
                  <a href="#" className="text-[14px] font-bold flex items-center justify-center gap-1 hover:opacity-75 transition-opacity" style={{ color: brandGreen }}>
  ...
  <div className="max-w-[1440px] mx-auto text-center flex flex-col items-center">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-8 w-full">
        ...
        <div className="max-w-[1440px] mx-auto flex flex-col lg:flex-row items-center justify-between w-full relative">
          <div className="w-full lg:w-[45%] relative flex justify-center lg:justify-start mb-10 lg:mb-0 z-10">
            <div className="relative w-full flex justify-center">
              <img 
                src={mockupImg} 
                alt="Great Clips App Mockup" 
                className="relative lg:absolute w-[280px] md:w-[350px] lg:w-auto h-auto lg:h-[550px] max-w-none object-contain 
                           drop-shadow-[0_50px_60px_rgba(0,0,0,0.45)] 
                           scale-[1.3] md:scale-[1.4] lg:scale-[1.9] 
                           top-0 lg:top-1/2 transform lg:-translate-y-1/2 
                           left-0 lg:left-0 lg:-translate-x-[15%]"
              />
            </div>
          </div>
      ...
      {/* 5. NEW GREAT NEWS SECTION */}
      <section className="bg-[#f4f9f7] py-24 px-6 mt-20 lg:mt-40">
  <div className="max-w-[1440px] mx-auto">
    <h2 className="text-[40px] md:text-[50px] font-black text-gray-900 mb-16 text-center">
      Great news
    </h2>
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
      {newsItems.map((item, index) => (
        <div 
          key={index} 
          className="flex flex-col group cursor-pointer text-left transition-all duration-500 hover:-translate-y-2"
        >
          {/* Image Container with Zoom & Overlay effect */}
          <div className={`relative aspect-[16/9] md:aspect-[1.8/1] rounded-[1.5rem] overflow-hidden mb-8 shadow-sm group-hover:shadow-xl transition-all duration-500 ${item.bgColor}`}>
            <img 
              src={item.image} 
              alt={item.title}
          ...
          <div className="flex flex-col px-1">
            <h3 className="text-[26px] font-black text-gray-900 mb-3 leading-tight tracking-tight group-hover:text-[#006A52] transition-colors duration-300">
              {item.title}
            </h3>
            <p className="text-gray-700 text-[18px] font-medium leading-[1.6] mb-6">
              {item.description}
            </p>
            
            {/* Animated Link */}
            <div className="flex items-center gap-1.5 text-[#006A52] font-extrabold text-[16px]">
              <span className="relative after:content-[''] after:absolute after:w-full after:scale-x-0 after:h-0.5 after:bottom-0 after:left-0 after:bg-[#006A52] after:origin-bottom-right after:transition-transform after:duration-300 group-hover:after:scale-x-100 group-hover:after:origin-bottom-left">
                {item.linkText}
              </span>
